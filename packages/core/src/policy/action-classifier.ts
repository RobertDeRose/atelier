import type { ActionKind, OperationRisk } from "../domain/types.ts";
...
function classifyGit(args: string[], rationale: string[]): CommandClassification {
  ...
  if (subcommand === "branch") {
    ...
    return mutation("repository.change.create", rationale, false, false, command, "high", risk);
  }
  if (subcommand === "tag") {
    const tail = args.slice(2);
  ...
  if (destructive.has(subcommand) || (subcommand === "checkout" && args.includes("--"))) {
    ...
    return mutation("repository.change.create", rationale, false, false, command, "high", "destructive");
  }
  if (["fetch", "ls-remote", "pull", "push", "remote"].includes(subcommand)) {
    ...
    return mutation(action, rationale, false, true, command, "high", "external");
  }
  rationale.push(`git ${subcommand || "command"} can mutate repository state`);
  return mutation("repository.change.create", rationale, false, false, command);
}
...
function classifyJj(args: string[], rationale: string[]): CommandClassification {
  ...
  if (subcommand === "workspace") {
    ...
    return mutation("repository.workspace.create", rationale, false, false, command, "high", risk);
  }
  ...
  if (subcommand === "git" && args[2] === "push") {
    ...
    return mutation("repository.publish", rationale, false, true, command, "high", "external");
  }
...
function classifyBeads(args: string[], rationale: string[]): CommandClassification {
  ...
  if (["blocked", "children", "dep", "graph", "info", "list", "prime", "ready", "show", "status", "version", "where"].includes(subcommand)) {
    if (subcommand === "dep" && !["", "cycles", "list", "tree"].includes(args[2] ?? "")) {
      ...
      return mutation("task.link", rationale, false, false, args.join(" "));
    }
  ...
  if (subcommand === "create" || subcommand === "init") {
    ...
    return mutation("task.create", rationale, false, false, args.join(" "));
  }
  if (["close", "defer", "supersede"].includes(subcommand)) {
    rationale.push(`bd ${subcommand} changes task lifecycle state`);
    return mutation("task.close", rationale, false, false, args.join(" "));
  }
  rationale.push(`bd ${subcommand || "command"} can mutate task state`);
  return mutation("task.update", rationale, false, false, args.join(" "));
}
...
function classifyPackageManager(executable: string, args: string[], rationale: string[]): CommandClassification {
  ...
  if (["install", "add", "remove", "uninstall", "update", "upgrade", "link", "unlink", "dedupe"].includes(subcommand)) {
    ...
    return mutation("dependency.modify", rationale, true, true, args.join(" "), "high", risk);
  }
  if (["test", "check", "lint", "run", "exec", "x", "dlx"].includes(subcommand)) {
    ...
    return mutation("command.long_running", rationale, true, subcommand === "dlx", args.join(" "));
  }
  rationale.push(`${executable} command is not proven read-only`);
  return mutation("command.execute", rationale, false, NETWORK_EXECUTABLES.has(executable), args.join(" "), "low");
}
...
function classifyMise(args: string[], rationale: string[]): CommandClassification {
  ...
  if (["run", "exec", "x"].includes(subcommand)) {
    ...
    return mutation("command.long_running", rationale, true, false, args.join(" "));
  }
  if (["install", "uninstall", "upgrade", "use"].includes(subcommand)) {
    ...
    return mutation("network.access", rationale, true, true, args.join(" "), "high", "external");
  }
  rationale.push("mise command is not proven repository-local");
  return mutation("command.execute", rationale, false, false, args.join(" "), "low", "unknown");
}
...
function classifyFind(args: string[], command: string, rationale: string[]): CommandClassification {
  ...
  if (mutatingFlag !== undefined) {
    ...
    return mutation("write.file", rationale, false, false, command, "high", risk);
  }
  ...
  for (let index = 1; index < args.length; index += 1) {
    ...
    if (invoked.length === 0 || index >= args.length) {
      return mutation("command.execute", ["find execution clause could not be bounded safely"], false, false, command, "low");
    }
...
function classifyFd(args: string[], command: string, rationale: string[]): CommandClassification {
  ...
  if (executionIndex !== -1) {
    ...
    const classified = nested.length === 0
      ? mutation("command.execute", ["fd execution clause is incomplete"], false, false, command, "low")
...
function classifyRipgrep(args: string[], command: string, rationale: string[]): CommandClassification {
  if (args.some((arg) => arg === "--pre" || arg.startsWith("--pre="))) {
    ...
    return mutation("command.execute", rationale, false, false, command, "high", "unknown");
  }
...
function classifySed(args: string[], command: string, rationale: string[]): CommandClassification {
  if (args.some((arg) => arg === "-i" || arg.startsWith("-i") || arg === "--in-place" || arg.startsWith("--in-place="))) {
    ...
    return mutation("write.file", rationale, false, false, command);
  }
...
function classifySimpleCommand(command: string): CommandClassification {
  ...
  if (!command.trim()) {
    return mutation("command.execute", ["empty command is not executable"], false, false, command, "low");
  }
  ...
  try {
    ...
    return mutation("command.execute", ["command could not be parsed safely"], false, false, command, "low");
  }
  const executable = (args[0] ?? "").split("/").at(-1)?.toLowerCase() ?? "";
  if (!executable) return mutation("command.execute", ["missing executable"], false, false, command, "low");
  ...
  if (["bash", "bun", "node", "python", "python3", "ruby", "tsx"].includes(executable)) {
    ...
    return mutation("command.execute", rationale, false, false, command);
  }
  ...
  if (["cargo", "go", "make", "mvn", "gradle", "gradlew", "pytest", "tox"].includes(executable)) {
    ...
    return mutation("command.long_running", rationale, true, false, command, "medium");
  }
  if (NETWORK_EXECUTABLES.has(executable)) {
    rationale.push(`${executable} may access the network`);
    return mutation("network.access", rationale, false, true, command, "medium", "external");
  }
  rationale.push("command is not proven read-only");
  return mutation("command.execute", rationale, LONG_RUNNING_EXECUTABLES.has(executable), false, command, "low", "unknown");
}
