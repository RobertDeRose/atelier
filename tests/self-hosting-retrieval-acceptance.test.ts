...
class SelfHostingProvider implements CodeProvider {
  ...
  async search(query: CodeSearchQuery): Promise<CodeSearchHit[]> {
    ...
    const documents = [
      ...
      { repositoryId: "docs", path: "docs/src/features/canonical-retrieval-planning/code-intelligence/index.md", symbol: "CodeIntelligenceGuide" },
...
test("self-hosting planning stays within eight intelligence calls without stale or cross-scope evidence", async () => {
  ...
  assert.ok(expectedPaths.has("docs/src/features/canonical-retrieval-planning/code-intelligence/codesearch/evaluation.md"));
