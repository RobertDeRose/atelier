...
Current release: **0.14.0-alpha.37**. Atelier retains the canonical path-identity and portable alias-test contract delivered in alpha.33. Alpha.34 corrects the live-acceptance implementation gate: Pi read errors are correlated to their exact tool-call arguments, an `EISDIR` result is tolerated only for a directory inside the approved workspace, and the expected `ENOENT` allowance remains limited to the missing focused test file. The live harness also supports idempotent `resume implementation`, so a completed model edit is not repeated after a harness-only assertion failure.
